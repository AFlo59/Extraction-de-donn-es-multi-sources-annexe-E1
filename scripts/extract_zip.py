...
def extract_zip():
    ...
    try:
        ...
        if os.path.exists(local_zip_path):
            ...
            if remote_size == local_size:
                logging.info(f"Aucune mise à jour pour {zip_file_name}.")
            else:
                logging.info(f"Mise à jour détectée pour {zip_file_name}. Téléchargement du nouveau fichier.")
                updated = True
                # Télécharger le fichier ZIP
                with fs.open(zip_path, 'rb') as remote_file:
                    with open(local_zip_path, 'wb') as local_file:
        ...
        if updated:
            ...
            with zipfile.ZipFile(local_zip_path, 'r') as zip_ref:
                zip_ref.extractall(output_dir)
                logger.info(f"Fichier ZIP {zip_file_name} extrait dans {output_dir}")
            logging.info(f"Fichier ZIP {zip_file_name} extrait dans {output_dir}")
